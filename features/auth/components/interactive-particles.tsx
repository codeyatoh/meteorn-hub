...
void main() {
  ...
  // Preserve the original color of the image, multiplied by uColor for optional tinting.
  vec3 rgb = colA.rgb * uColor;
  
  // Use the image's original alpha to handle transparent PNGs correctly
  float alpha = t * colA.a;

  // IMPORTANT: WebGL uses premultiplied alpha by default! 
  // We MUST multiply rgb by alpha before outputting, otherwise the browser will clamp 
  // bright colors (like white/yellow) to grey when blending with a white background!
  ...
  useEffect(() => {
    ...
    loader.load(effectiveSrc, (texture) => {
      ...
      for (let i = 0; i < numPoints; i++) {
        ...
        let keep = false;
        if (isOpaqueImage) {
          const luminance = 0.21 * r + 0.71 * g + 0.07 * b;
          keep = luminance > threshold;
        } else {
          keep = a > threshold;
        }
